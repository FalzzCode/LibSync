...
<section class="page">
    ...
    <div class="developer-grid">
        <section class="recent-panel"><div class="recent-panel__heading"><div><p class="eyebrow">Aksi pengujian</p><h2>Tools aman</h2></div></div><div class="developer-actions"><form method="POST" action="{{ route('developer.seed-demo') }}">@csrf<button class="btn btn--primary">Siapkan data uji</button><small>Membuat data contoh hanya bila belum ada.</small></form><form method="POST" action="{{ route('developer.check-overdues') }}">@csrf<button class="btn btn--secondary">Jalankan cek terlambat</button><small>Menjalankan scheduler sekarang.</small></form><form method="POST" action="{{ route('developer.clear-cache') }}">@csrf<button class="btn btn--secondary">Bersihkan cache</button><small>Refresh config, route, dan view cache.</small></form></div></section>
