...
<section class="page">
    ...
    <form class="filter-bar" method="GET" action="{{ route('fines.index') }}">
        <div class="search-field"><label class="sr-only" for="fineSearch">Cari denda</label><span aria-hidden="true" data-solar-icon="solar:magnifer-linear">⌕</span><input id="fineSearch" type="search" name="search" value="{{ request('search') }}" placeholder="Cari nama anggota atau judul buku…" spellcheck="false"><button class="search-field__clear" type="button" data-search-clear aria-label="Hapus pencarian" hidden><span data-solar-icon="solar:close-circle-linear" aria-hidden="true">×</span></button></div>
